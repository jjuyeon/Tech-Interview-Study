...
## Question
1. [쓰레드란 무엇이고, 싱글쓰레드와 멀티쓰레드의 차이를 설명해주세요.](#1-1-single-thread)
- 스레드는 프로세스로부터 자원을 할당받아서 프로세스 내부에서 실행되는 작업 흐름의 단위를 의미하며, 다른 스레드와 공간, 자원을 공유하면서 사용됩니다.
- 싱글스레드는 하나의 프로세스에 하나의 스레드가 실행되는 것을 의미하지만, 멀티스레드는 하나의 프로세스에 여러 개의 스레드가 실행되는 것을 의미합니다.
<br><br>

2. [클래스는 무엇이고, 객체는 무엇인지 설명해주세요.](#2-클래스-vs-객체-vs-인스턴스)
- 클래스는 객체를 만들어 내기 위한 ‘설계도’로 객체를 정의해 놓은 것을 의미하며, 객체는 ‘설계도로 구현한 모든 대상’으로 클래스에 선언된 모양 그대로 생성된 실체를 의미합니다.
<br><br>

3. [인터페이스와 추상클래스의 차이점은 무엇인지 설명해주세요.](#3-1-인터페이스-vs-추상-클래스)
- 인터페이스는 상속받을 자식 클래스에게 구현할 메서드들의 원형을 모두 알려주어, 자식 클래스가 자신의 목적에 맞게 메서드를 구현하도록 하는 것을 의미합니다. 
- 반면, 추상클래스는 기존의 클래스에서 공통된 부분을 추상화(추상 메소드)하여 상속하는 클래스에게 구현을 강제화하는 것을 의미합니다.
- 큰 차이점으로는, 인터페이스는 클래스가 아니기 때문에 단일 상속과 다중 상속이 가능하지만, 추상클래스는 클래스이므로 단일 상속만이 가능합니다.
<br><br>

4. [직렬화가 무엇인지 설명하세요.](#4-직렬화serialize)
- 직렬화는 객체 또는 데이터를 외부의 파일에 저장하거나 네트워크를 통해 전달 할 수 있도록 byte 형태로 변환하는 기술을 의미합니다.
<hr>
